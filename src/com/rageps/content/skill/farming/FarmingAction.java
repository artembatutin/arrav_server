...
import com.rageps.content.skill.farming.seed.impl.FlowerSeed;
			...
			ObjectAction obj = new ObjectAction() {
				...
				public boolean click(Player player, GameObject object, int option) {
					...
					if(farming.patch != null && farming.patch.getSeedType() != null) {
						...
						final String stateMessage = farming.patch.getGrowthState() != GrowthState.WATERED ? farming.patch.getSeedType() != null && (farming.patch.getSeedType().getSeedClass() == BUSHES || farming.patch.getSeedType().getSeedClass() == TREES || farming.patch.getSeedType().getSeedClass() == MUSHROOMS || farming.patch.getSeedType() == FlowerSeed.WHITE_LILY) ? "The crops are looking healthy." : "The crops are looking healthy; should probably water them to keep it that way!" : "The crops are looking healthy and watered!";
						if(farming.patch.getSeedType().getSeedClass() == TREES) {
							if(option == 1) {
								...
								if(farming.patch.getGrowthState() == GrowthState.DISEASED) {
									if(farming.patch.getSeedType().getSeedClass() == TREES || farming.patch.getSeedType().getSeedClass() == BUSHES) {
									...
									String cure = farming.patch.getSeedType().getSeedClass() == TREES || farming.patch.getSeedType().getSeedClass() == BUSHES ? "a pair of secateurs" : "a cure plant potion";
						...
						if(option == 4) {
							...
							return true;
						} else if(option == 2 && farming.patch.getSeedType().getSeedClass() == BUSHES) {
							//inspect
							if(farming.patch.getGrowthState() == GrowthState.DISEASED) {
			...
			ItemOnObjectAction item = new ItemOnObjectAction() {
				...
				public boolean click(Player player, GameObject object, Item item, int container, int slot) {
					...
					if(item.getId() >= 5333 && item.getId() <= 5340 || magicWateringCan) {
						...
						if(patch.getSeedType().getSeedClass() == TREES || patch.getSeedType().getSeedClass() == BUSHES) {
						...
						if(patch.getSeedType().getSeedClass() != TREES && patch.getSeedType().getSeedClass() != BUSHES)
							return true;
						if(patch.getGrowthState() != GrowthState.DISEASED) {
							String name = patch.getSeedType().getSeedClass() == TREES ? "tree" : "bush";
					...
					if(seedType != null) {
						if(patch == null) {
							...
							return true;
						} else if(seedType.getSeedClass() != TREES && !player.getInventory().contains(FarmingConstants.SEED_DIBBER_ITEM_ID)) {
							player.message("You need a seed dibber to plant these seeds.");
							return true;
						} else if(seedType.getSeedClass() == TREES && !player.getInventory().contains(FarmingConstants.TROWEL_ITEM_ID)) {
						...
						(new Task(2, false) {
							...
							protected void execute() {
								...
								if(finalSeedType.getSeedClass() == TREES) {
