...
import com.rageps.content.skill.construction.data.Butlers;
...
public class House {
	...
	public void addPlayer(Player visitor) {
		...
		if(visitors.isEmpty()) {
			...
			World.get().getMobRepository().add(servant);
			addNpc(servant);
		}
		visitor.setInstance(instance);
		visitors.add(visitor);
		if(visitor.same(owner))
			visitor.getHouse().get().setState(HouseController.State.HOME);
		else
			visitor.getHouse().get().setState(HouseController.State.VISITING);
	...
	public void removePlayer(Player player) {
		...
		player.getHouse().get().setState(HouseController.State.AWAY);
		if(visitors.isEmpty()) {
	...
	public boolean isOwnerHome() {
		return owner.getHouse().get().getState() == HouseController.State.HOME || owner.getHouse().get().getState() == HouseController.State.HOME_DUNGEON;
	}
}