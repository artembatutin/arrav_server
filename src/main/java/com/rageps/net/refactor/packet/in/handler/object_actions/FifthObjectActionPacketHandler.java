...
import static com.rageps.action.ActionContainers.CONSTRUCTION;
import static com.rageps.action.ActionContainers.OBJECT_FIFTH;

/**
 * @author Tamatea <tamateea@gmail.com>
 */
public class FifthObjectActionPacketHandler implements PacketHandler<FifthActionObjectPacket> {

    @Override
    public void handle(Player player, FifthActionObjectPacket packet) {
            ...
            if(e != null) {
                ...
                player.getMovementListener().append(() -> {
                    if(boundary.within(player.getPosition(), player.size(), 1)) {
                        e.click(player, new DynamicObject(objectId, new Position(objectX, objectY, player.getPosition().getZ()), ObjectDirection.SOUTH, ObjectType.GENERAL_PROP, false, 0, player.getInstance()), 5);
                    }
                });
            }
        }
        ...
        player.getMovementListener().append(() -> {
            if (distanceIgnore || boundary.within(player.getPosition(), player.size(), 1)) {
                ...
                if(!MinigameHandler.execute(player, m -> m.onFifthClickObject(player, object)))
                    return;
    ...
    private boolean event(Player player, GameObject object) {
        ObjectAction e = OBJECT_FIFTH.get(object.getId());
        if(e != null) {
            return e.click(player, object, 5);
        }
        return false;
    }
