 ...
 */
public class ThirdObjectActionPacketHandler implements PacketHandler<ThirdActionObjectPacket> {

    @Override
    public void handle(Player player, ThirdActionObjectPacket packet) {
            ...
            if(e != null) {
                ...
                player.getMovementListener().append(() -> {
                    if(boundary.within(player.getPosition(), player.size(), 1)) {
                        e.click(player, new DynamicObject(objectId, new Position(objectX, objectY, player.getPosition().getZ()), ObjectDirection.SOUTH, ObjectType.GENERAL_PROP, false, 0, player.getInstance()), 3);
                    }
                });
            }
        }
        ...
        if(player.getRights().equals(Rights.ADMINISTRATOR))
            player.message("[OBJ 3]:" + object.getId() + " - " + object.getPosition().toString());
    ...
    private boolean event(Player player, GameObject object) {
        ObjectAction e = OBJECT_THIRD.get(object.getId());
        if(e != null) {
            return e.click(player, object, 3);
        }
        return false;
    }
