 ...
 */
public class FirstObjectActionPacketHandler implements PacketHandler<FirstActionObjectPacket> {

    @Override
    public void handle(Player player, FirstActionObjectPacket packet) {
            ...
            if(e != null) {
                ...
                player.getMovementListener().append(() -> {
                    if(boundary.within(player.getPosition(), player.size(), 1)) {
                        e.click(player, new DynamicObject(objectId, new Position(objectX, objectY, player.getPosition().getZ()), ObjectDirection.SOUTH, ObjectType.GENERAL_PROP, false, 0, player.getInstance()), 1);
                    }
                });
            }
        }
    ...
    private boolean event(Player player, GameObject object) {
        ObjectAction e = OBJECT_FIRST.get(object.getId());
        if(e != null) {
            return e.click(player, object, 1);
        }
        return false;
    }
