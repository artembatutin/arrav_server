...
import static com.rageps.world.entity.actor.player.assets.Rights.*;

/**
 * Handles the claim donate process for a specific player.
 */
public class Punishments extends ConnectionUse {
	 ...
	 */
	private static final Logger logger = Logger.getLogger(Punishments.class.getName());
	 ...
	 */
	public Punishments(Player player, ConnectionPool pool) {
		super(pool);
		this.player = player;
	}
				...
				switch(player.getRights()) {
					...
					case EXTREME_DONATOR:
						if(player.getTotalDonated(true) > 1000) {
							player.setRights(GOLDEN_DONATOR);
