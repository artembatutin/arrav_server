...
/**
 * The area type that models any area in a circle or oval shape.
...
public final class CircleArea extends Area {
	...
	/**
	 * The radius of this area from the center coordinates.
	 */
	 ...
	 */
	public CircleArea(int x, int y, int z, int radius) {
		this.x = x;
		this.y = y;
		this.z = z;
		this.radius = radius;
	...
	public boolean equals(Object obj) {
		...
		if(!(obj instanceof CircleArea))
			return false;
		CircleArea other = (CircleArea) obj;
	...
	public boolean isWithin(Position position, int distance) {
		if(position.getZ() != z)
			return false;
		return Math.pow((position.getX() - x), 2) + Math.pow((position.getY() - y), 2) <= Math.pow(radius + distance, 2);
	}
	 ...
	 */
	public CircleArea copy() {
		return new CircleArea(x, y, z, radius);
	}
	...
	/**
	 * Gets the radius of this area from the center coordinates.
	 * @return the radius of this area from the center coordinates.
	 */
