...
public final class TradeSession extends ExchangeSession {
	...
	public void updateMainComponents() {
		if(getStage() == OFFER_ITEMS) {
			this.getPlayers().forEach(player -> {
				...
				player.send(new ItemsOnInterfacePacket(player, 3415, this.getExchangeSession().get(player)));
				player.send(new ItemsOnInterfacePacket(player, 3416, this.getExchangeSession().get(player)));
			...
			this.getPlayers().forEach(player -> {
				...
				player.send(new ItemsOnInterfacePacket(player, 3214, player.getInventory()));
	...
	public void updateOfferComponents() {
		for(Player player : this.getExchangeSession().keySet()) {
			...
			player.send(new ItemsOnInterfacePacket(player, 3322, player.getInventory()));
			player.send(new ItemsOnInterfacePacket(player, 3415, this.getExchangeSession().get(player)));
			recipient.send(new ItemsOnInterfacePacket(recipient, 3416, this.getExchangeSession().get(player)));
			player.interfaceText(3431, "");
			player.interfaceText(3417, "Trading with: " + name(recipient) + " " + "who has @gre@" + remaining + " free slots");
		}
	}
