...
public abstract class ByteBuf implements ReferenceCounted, Comparable<ByteBuf> {
	 ...
	 */
	private GamePacketType msgType;
	/**
	...
	/**
	 * Prepares this {@link ByteBuf} as an incoming buffer.
	 * @param opcode packet opcode.
	 * @param type packet type.
	 * @return instance of this buffer.
	 */
	public ByteBuf incoming(int opcode, GamePacketType type) {
		this.opcode = opcode;
		this.msgType = type;
		return this;
	}
	...
	/**
	 * Prepares this {@link ByteBuf} as an outgoing buffer.
	 * @param encryptor isaac cypher.
	 * @return instance of this buffer.
	 */
	public ByteBuf outgoing(IsaacRandom encryptor) {
		this.encryptor = encryptor;
		return this;
	}
	...
	public void message(int id, GamePacketType type) {
		...
		writeByte(id + encryptor.nextInt());
		sizeIndex = writerIndex();
		msgType = type;
	...
	public void endVarSize() {
		int recordedSize = writerIndex() - sizeIndex;
		if(msgType == GamePacketType.VARIABLE_BYTE) {
			setByte(sizeIndex, recordedSize - 1);
		} else if(msgType == GamePacketType.VARIABLE_SHORT) {
	...
	public String getCString() {
		...
		while(isReadable() && (temp = (byte) readUnsignedByte()) != TERMINATOR_VALUE) {
			b.append((char) temp);
		}
		return b.toString();
	}
	...
	public byte[] getBytesReverse(int amount, ByteTransform type) {
		byte[] data = new byte[amount];
		int dataPosition = 0;
		for(int i = readerIndex() + amount - 1; i >= readerIndex(); i--) {
			int value = getByte(i);
			switch(type) {
				case A:
					value -= 128;
