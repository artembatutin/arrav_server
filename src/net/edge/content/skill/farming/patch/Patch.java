		...
		if(getSeedType() != null && getSeedType().getSeedClass() == SeedClass.ALLOTMENT) {
			...
			if(otherPatch != null && otherPatch.getSeedType() != null) {
				...
				if(seedId == FlowerSeed.WHITE_LILY.getSeed().getId()) {
					...
					protectedPatch = getSeedType().getSeed().getId() == AllotmentSeed.ONION.getSeed()
							.getId() || getSeedType().getSeed().getId() == AllotmentSeed.TOMATO.getSeed()
							.getId() || getSeedType().getSeed().getId() == AllotmentSeed.POTATO.getSeed().getId();
	...
	public boolean isProtected(Player player) {
		...
		if(otherPatch != null && otherPatch.getSeedType() != null) {
			...
			if(seedId == FlowerSeed.WHITE_LILY.getSeed().getId()) {
				...
				return getSeedType().getSeed().getId() == AllotmentSeed.ONION.getSeed()
						.getId() || getSeedType().getSeed().getId() == AllotmentSeed.TOMATO.getSeed()
						.getId() || getSeedType().getSeed().getId() == AllotmentSeed.POTATO.getSeed().getId();
