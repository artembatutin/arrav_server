package net.arrav.content.skill.hunter;
...
import net.arrav.content.skill.hunter.trap.TrapProcessor;
import net.arrav.content.skill.hunter.trap.TrapTask;
...
public final class Hunter {
	...
	public static Optional<Trap> getTrap(Player player, GameObject object) {
		...
		for(Trap t : GLOBAL_TRAPS.get(player).getTraps()) {
			if(t.getObject().getId() == object.getId() && t.getObject().getPosition().same(object.getPosition())) {
