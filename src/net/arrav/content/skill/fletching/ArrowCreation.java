package net.arrav.content.skill.fletching;
...
import static net.arrav.content.achievements.Achievement.ARCHER_SUPPORTER;
...
public final class ArrowCreation extends ProducingSkillAction {
	...
	public boolean init() {
		...
		if(player.getInventory().computeAmountForId(definition.tips.getId()) < 15 || player.getInventory().computeAmountForId(HEADLESS_ARROW.getId()) < 15) {
			player.message("You must have atleast 15 of each item to fletch this arrow.");
			return false;
		}
		return true;
	}
	...
	private enum ArrowData {
		...
		ABYSSALBANE_ARROWS(21838, 21655, 76, 150),
		BASILISKBANE_ARROWS(21833, 21650, 76, 150),
		DRAGONBANE_ARROWS(21823, 21640, 76, 150),
		WALLASALKIBANE_ARROWS(21828, 21645, 76, 150),
	...
	public static class HeadlessArrowCreation extends ProducingSkillAction {
		...
		@Override
		public boolean init() {
			if(player.getInventory().computeAmountForId(toRemove.getId()) < 15 || player.getInventory().computeAmountForId(FEATHER.getId()) < 15) {
				player.message("You must have atleast 15 of each item to fletch a headless arrow.");
				return false;
			}
			return true;
		}
