package net.arrav.fs.cache.impl;
...
public final class CacheArchive {
	...
	public static CacheArchive decode(ByteBuf data) throws IOException {
		...
		if(compressedLength != length) {
			uncompressedData = Misc.unbzip2Headerless(data.array(), Cache.INDEX_SIZE, compressedLength);
		...
		for(int i = 0; i < total; i++) {
			...
			length = Misc.getMedium(data);
			compressedLength = Misc.getMedium(data);

			byte[] sectorData = new byte[length];

			if(length != compressedLength) {
				sectorData = Misc.unbzip2Headerless(uncompressedData, offset, compressedLength);
			} else {
