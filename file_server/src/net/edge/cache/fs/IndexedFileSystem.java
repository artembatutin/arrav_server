...
public final class IndexedFileSystem implements Closeable {
	 ...
	 */
	public IndexedFileSystem(File base) throws Exception {
		detectLayout(base);
	}
	...
	private void detectLayout(File base) throws Exception {
		int indexCount = 0;
		String path = base.getAbsolutePath().replaceAll("/file_server", "");
		for(int index = 0; index < indices.length; index++) {
			File f = new File(path + "/main_file_cache.idx" + index);
		...
		data = new RandomAccessFile(new File(path + "/main_file_cache.dat"), "r");
	}
